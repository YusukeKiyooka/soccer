...
for i in range(image_count):
  img = cv2.imread(input_data_path + '/' + str(i) + '.jpeg',cv2.IMREAD_COLOR )
  gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
  face = faceCascade.detectMultiScale(gray, 1.3, 3)
  if len(face) > 0:
    for rect in face:
      ...
      w = rect[2]
      h = rect[3]
      cv2.imwrite(save_path + '/messi' + str(face_detect_count) + '.jpg', img[y:y+h, x:x+w])
